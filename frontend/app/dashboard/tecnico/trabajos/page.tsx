...
const estadoConfig: Record<string, { label: string; variant: "default" | "secondary" | "destructive" | "outline"; icon: React.ElementType }> = {
  in_progress: { label: "Asignados", variant: "secondary", icon: AlertCircle },
...
export default function TecnicoTrabajos() {
  ...
  const navigation = [
    { name: "Inicio", href: "/dashboard/tecnico", icon: Home, current: false },
    { name: "Trabajos", href: "/dashboard/tecnico/trabajos", icon: Briefcase, current: true },
          ...
          <nav className="flex-1 space-y-1 p-4">
            {navigation.map((item) => (
              <Link key={item.name} href={item.href}
                className={`flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-colors ${item.current
        ...
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-border bg-card px-4 sm:px-6">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden sm:block">
              <h1 className="text-lg font-semibold text-foreground">Mis Trabajos Asignados</h1>
            </div>
          </div>
          <div className="flex items-center gap-3">
            ...
            <CardHeader>
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <CardDescription>Aquí puedes ver y gestionar todos los trabajos que te han sido asignados</CardDescription>
                </div>
              ...
              <Tabs value={activeTab} onValueChange={setActiveTab}>
                <TabsList className="mb-4">
                  ...
                  <TabsTrigger value="en_progreso" className="gap-2">
                    Asignados
                    {!loading && trabajos.filter(t => t.status === 'in_progress').length > 0 && (
                      <Badge variant="secondary" className="ml-1">{trabajos.filter(t => t.status === 'in_progress').length}</Badge>
                    )}
                  </TabsTrigger>
                ...
                {["todos", "en_progreso", "completados"].map(tabValue => (
                  <TabsContent key={tabValue} value={tabValue} className="mt-0">
                    {loading ? (
                      ...
                      tabValue === "todos" ? true :
                        tabValue === "en_progreso" ? t.status === "in_progress" :
                                  ...
                                  {trabajo.client && (
                                    <div className="flex items-center gap-2 mt-2">
                                      <Avatar className="h-6 w-6">
                                        <AvatarFallback className="text-[10px]">{trabajo.client.name?.charAt(0) || "C"}</AvatarFallback>
                                      </Avatar>
